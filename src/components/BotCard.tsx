  ...
  if (variant === "horizontal") {
    return (
      <Link to={`/bot/${bot.id}`} className="block">
        <Card className="bg-slate-800/60 border-slate-700/40 backdrop-blur-sm hover:bg-slate-800/80 transition-all duration-300 cursor-pointer group overflow-hidden h-28">
          <CardContent className="p-4 h-full">
            <div className="flex gap-4 h-full">
              {/* Avatar */}
              <div className="flex-shrink-0">
                <Avatar className="w-20 h-20 ring-1 ring-slate-600/30">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex items-center gap-1">
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      className={`h-6 w-6 p-0 ${isLiked ? 'text-red-500 hover:text-red-600' : 'text-slate-400 hover:text-slate-300'}`}
